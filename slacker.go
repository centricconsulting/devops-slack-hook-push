...
func LoadSlackers() bool {
	...
	// Run through the slackers and see if there are any things we need to set like
	// the system var.
	for key, value := range slackers {
		if value.IsSystem {
			systemKey = key
			log.Printf("info: System slacker set to %s", key)
		}
	} // for
...
func MakeSystemSlacker(params martini.Params) (int, string) {
	...
	// Set the system.
	tmp := slackers[params["key_id"]]
	tmp.IsSystem = true
	slackers[params["key_id"]] = tmp
	systemKey = params["key_id"]
	// There can be only one.
	for key, value := range slackers {
		if key != params["key_id"] && value.IsSystem {
			tmp := slackers[params["key_id"]]
			tmp.IsSystem = false
			slackers[params["key_id"]] = tmp
		}
	} // for
	return http.StatusOK, "System key assigned."
} // func
